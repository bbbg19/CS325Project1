...
def tspRun(inputValues):
    path =[] 
    distanceCovered = 0 
    sortedPoints = sorted(inputValues, key=distGetter)
    ...
    while len(sortedPoints) > 0: 
        if endCheck == 0:
            ...
            two = distance(path[0], sortedPoints[len(sortedPoints) - 2]) 
            ...
            if one < two:
                ...
                path.insert(0, sortedPoints[len(sortedPoints) - 2])
                del sortedPoints[len(sortedPoints) - 2]
                distanceCovered += two
                
        if endCheck == 1:
            one = distance(path[-1], sortedPoints[-1]) #gets the distance to the next furthest point
            two = distance(path[-1], sortedPoints[len(sortedPoints) - 2]) #gets the distance to the second furthest point
            ...
            if one < two:
                ...
                path.append(sortedPoints[len(sortedPoints) - 2])
                del sortedPoints[len(sortedPoints) - 2]
                distanceCovered += two
        if endCheck == 0:
            endCheck = 1
